"""
Generate the BENG 2800 final report (Apr 5 version) with ALL issue fixes.

Issue 1: Separate untuned from broadly tuned; clarify fit convergence
Issue 2: Reframe reliability-sharpness with simulation context
Issue 3: Random-neuron decoder as primary; top-1000 as upper bound; overfitting discussion
Issue 4: Clarify binned R² vs single-trial
Issue 5: Soften language, add caveats

Outputs to reportsApr5/ folder.
"""
import os
import numpy as np
import pandas as pd
...
OUT = os.path.join(BASE, "reportsApr5")
...
def main():
    print("Generating Apr 5 report with all issue fixes...\n")
    ...
    body(doc,
        ...
        "Understanding how orientation is encoded across large neural populations remains "
        "a central question in systems neuroscience, with implications for sensory prosthetics "
    ...
    body(doc,
        ...
        "(bounded [0, 20]), and \u03b8\u2080 is preferred orientation. Goodness of fit was "
        "assessed using R\u00b2 computed on binned means. We note that R\u00b2 on trial-averaged "
        "tuning curves will be higher than single-trial predictive accuracy due to the noise "
        "reduction from averaging ~128 trials per bin.")
    ...
    body(doc,
        f"The OLS model (R\u00b2 = 0.055) confirmed the positive association (Table 1). The "
        f"modest R\u00b2 is consistent with the simulation analysis: the observed correlation is "
        f"weaker than the mathematically expected coupling, and \u03ba and mean response together "
        f"explain only 5.5% of reliability variance. Note that because reliability is bounded "
        f"[\u22121, 1], the OLS homoscedasticity assumption is violated; standard errors should be "
    ...
    for lim in [
        ...
        "Deconvolved calcium activity is an indirect measure of spiking that may bias tuning estimates.",
    ]:
    ...
    # Save
    out_path = os.path.join(OUT, "BENG2800_Final_Report_Apr5.docx")
    doc.save(out_path)
    print(f"Report saved: {out_path} ({os.path.getsize(out_path)/1e6:.2f} MB)")

    # Also copy key figures to reportsApr5 for easy reference
    import shutil
    for fig_name in [
